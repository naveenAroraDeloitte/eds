...
import {
  propertyChange, ExecuteRule, Initialize, RemoveItem, FormLoad, FieldChanged, ValidationComplete, Change, Valid, Invalid, SubmitSuccess, CustomEvent, SubmitError, SubmitFailure, Submit, RemoveInstance, AddInstance, Reset, AddItem, Click,
} from './afb-events.js';
import Formula from '../formula/index.js';
import {
  format, parseDefaultDate, datetimeToNumber, parseDateSkeleton, formatDate, numberToDatetime,
...
class DataGroup extends DataValue {
  ...
  constructor(_name, _value, _type = typeof _value) {
    ...
    if (_value instanceof Array) {
      this.$_items = _value.map((value, index) => this.createEntry(index, value));
    } else {
      this.$_items = Object.fromEntries(Object.entries(_value).map(([key, value]) => [key, this.createEntry(key, value)]));
    }
  }
  ...
  get $value() {
    ...
    return Object.fromEntries(Object.values(this.$_items).filter((x) => typeof x !== 'undefined').map((x) => [x.$name, x.$value]));
  }
...
const submit = async (context, success, error, submitAs = 'multipart/form-data', input_data = null) => {
  ...
  const submitDataAndMetaData = { data, submitMetadata: context.form.exportSubmitMetaData() };
